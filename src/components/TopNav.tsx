...
export function TopNav({ route }: { route: Route }) {
  ...
  // z-[60] keeps the bar above the rain overlay (z-50), so the pill still reads
  // as it fills while the weather rolls through underneath
  return (
    <div className="sticky top-3 z-[60] px-3 pb-3 sm:top-4 sm:px-6 sm:pb-4">
      <header
        className={cn(
...
function NavPill({
  ...
  return (
    ...
    >
      {/* the pill is a container like any other: water rises into it from the
          bottom when selected and drains back out when it isn't */}
      <LiquidFill filled={active} crestHeight={12} className="text-primary" />
